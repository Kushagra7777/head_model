...
def run_voting_ensemble(estimators, X_train, y_train, X_test, y_test, label_names, result_dir, seed):
    ...
    # Create a separate DataFrame for Correct and Wrong counts
    summary_df = pd.DataFrame(columns=results_df.columns)
    ...
    # Add these rows to the summary DataFrame
    summary_df.loc['Correct'] = [''] + correct_counts
    summary_df.loc['Wrong'] = [''] + wrong_counts

    # Save the final predicted vs actual comparison to CSV
    results_df.to_csv(result_dir / "predicted_vs_actual_full.csv", index=False)

    # Append the summary_df at the bottom of results_df
    final_results_df = pd.concat([results_df, summary_df])

    # Save the combined DataFrame
    final_results_df.to_csv(result_dir / "predicted_vs_actual_full.csv", index=True)
...
# python single_full.py --input_dir work_dir/model_ready_folders/setolabo_samples_as_test/PRJNA946800 --result_dir work_dir/model_ready_folders/setolabo_samples_as_test/PRJNA946800_result_full --seed 42 --estimator all --n_samples -1 --n_features -1
